...
The goal of artifact evaluation is to help science by ensuring that published papers are accompanied by high-quality artifacts (e.g., software, hardware, datasets, etc.) that can be reused and extended by others. Authors submit their artifacts (included in the artifact appendix) after the manuscript has been accepted (or conditionally accepted) for presentation at the conference. Moreover, the best artifacts that excel in all the evaluated dimensions will be rewarded with a **Distinguished Artifact Award**.
...
Afterwards, there is some time to agree on badges before the final deadline. This is to ensure that there is time for reviewers to discuss the artifacts that need it, as well as time for any extra or late evaluations. Keep in mind that the final deadline for agreeing on badges is strict.

All deadlines are at 23:59 AoE (Anywhere on Earth) time:
 * April 14, 2026: Artifact registration deadline
 * April 21, 2026: Artifact submission deadline
 * April 22 - 24, 2026: Artifact bidding
 * April 25 - May 2, 2026: Kick-the-tires stage
 * May 3 - 10, 2026: Preliminary artifact revision
 * May 11 - 27, 2026: Interactive phase
 * June 3, 2026: Badge notification to authors
 ...
 * Carefully read the artifact documentation. In particular, check the software and hardware dependencies to make sure you have all you need. You are allowed to use your own judgment when making decisions, for instance, to evaluate reasons why some artifacts may not be able to reproduce everything their paper contains. Before starting the evaluation, consider the following points and ideally share the evaluation plan with the authors:
    - Whether you have everything you need to do the evaluation, and if not, what is missing, including:
        ...
        - If the experiments are required to run in the physical world (e.g., flying drone tests), the authors choose to provide simulation-based artifacts for artifact evaluation
    ...
    - A plan on how you will evaluate the artifact during the review period:
        -Time frames for when experiments will be run in case hardware is shared
...
Once you finish evaluating an artifact, fill in the review form and submit it at your earliest convenience. Your review must explain in detail why the artifact should or should not get each of the badges that the authors requested. You can also include additional suggestions for the authors to improve their artifacts if you have any. Note that you can edit your review as many times as you like, since reviews only become visible to the authors when final decisions are announced.

Remember that the artifact evaluation process is cooperative, not adversarial. Give authors a chance to fix issues by discussing through HotCRP comments before deciding that their artifact should not get a badge. In other words, help the authors improve their artifacts and reach badge status in the allocated time, whenever possible. However, if authors are being unresponsive or unreasonable, feel free to post a comment stating that a badge cannot be awarded unless the authors take the specified steps in time by the deadline.

HotCRP allows you to rate your fellow evaluators’ reviews. Once you have submitted your review, you will be able to see other reviews on the same paper. You can help out with the overall review quality control by using the review tagging mechanism in the submission system, so please use this. At the bottom of each review, you will see the following buttons:
![rate-base](/vehiclesec2026/images/rating.png)

If you click on the ellipsis (triple dots) button, the line is expanded to:
![rate-ext](/vehiclesec2026/images/rating-extended.png)

If you think a review is well done, don’t hesitate to add a positive vote! By using these tags, you can help the chairs to identify the top reviewers and help improve the quality of the reviews overall. On the other hand, if you think a review could use improvement, you can leave a negative vote and a reviewer discussion comment explaining your thoughts.
